...
Key design points:
  * Cross-platform (Windows / Linux). Open Sans is the global UI typeface
    (bundled + registered at runtime: private GDI registration on Windows,
    per-user install via fontconfig on Linux). A monospace face is used ONLY
    for technical tokens (hotkeys / version) to improve differentiation.
  ...
  * Launching the binary only opens the control window; it NEVER auto-starts
    the service. The service must be started explicitly by the user.
  * Linux notes: global hotkeys need root privileges (the `keyboard` library
    reads /dev/input directly); screenshots use Pillow/XCB on X11 with
    graceful fallbacks to grim, gnome-screenshot, spectacle, scrot, maim or
    ImageMagick on Wayland-native sessions.
"""
...
def _base_dir():
    """Directory hosting the app (exe dir when frozen) - no CWD reliance."""
...
# Redirect stdout and stderr to solver_error.log (next to the app) to capture
# windowed crashes regardless of the current working directory.
try:
    log_file = open(os.path.join(_base_dir(), "solver_error.log"), "a",
                    encoding="utf-8")
    sys.stdout = log_file
    sys.stderr = log_file
except Exception:
...
# ----------------------------------------------------------------------------
# Open Sans ships bundled with the app and is registered at runtime:
#   * Windows: private GDI registration (AddFontResourceExW + FR_PRIVATE).
#   * Linux/macOS: per-user install into the fontconfig user directory
#     (~/.local/share/fonts or ~/Library/Fonts) followed by `fc-cache -f`.
# Justified exception: a monospace face is used exclusively for technical
# tokens (hotkeys, version badge): fixed-width aligned glyphs make key names
# such as "F8"/"F9" easier to scan. Consolas on Windows; DejaVu Sans Mono
# (or the best available monospace) elsewhere.
# ============================================================================
FONT_FAMILY = "Open Sans"
MONO_FAMILY = "Consolas" if IS_WINDOWS else "DejaVu Sans Mono"
...
_UI_FONT_FALLBACKS = ["DejaVu Sans", "Liberation Sans", "Noto Sans",
                      "FreeSans", "Ubuntu"]
_MONO_FONT_CANDIDATES = ["DejaVu Sans Mono", "Liberation Mono",
                         "Noto Sans Mono", "FreeMono", "Ubuntu Mono",
                         "monospace"]
...
def register_open_sans_fonts():
    """Make the bundled Open Sans TTFs available to Tk for this user."""
    global _fonts_registered
    if _fonts_registered:
        return True
    font_dir = get_resource_path(FONT_DIR_NAME)
    present = [f for f in _OPEN_SANS_FILES
            ...
            for fname in present:
                ...
                try:
                    res = gdi32.AddFontResourceExW(
                        ctypes.c_wchar_p(fpath), FR_PRIVATE, 0)
                    if res:
                        loaded_any = True
                except Exception:
                    pass
    ...
    # Linux / macOS: install into the per-user fonts directory so fontconfig
    # can resolve the "Open Sans" family even when it is not system-wide.
    if IS_MACOS:
        target_dir = os.path.join(os.path.expanduser("~"), "Library", "Fonts")
    else:
        data_home = (os.environ.get("XDG_DATA_HOME")
                     or os.path.expanduser("~/.local/share"))
        target_dir = os.path.join(data_home, "fonts", "TestSolverAI")
    try:
        os.makedirs(target_dir, exist_ok=True)
        for fname in present:
            shutil.copyfile(os.path.join(font_dir, fname),
                            os.path.join(target_dir, fname))
        if IS_LINUX and shutil.which("fc-cache"):
            subprocess.run(["fc-cache", "-f", target_dir],
...
def resolve_app_fonts(root):
    """Verify the configured families exist once Tk is up; fall back to the
    closest available system font otherwise (common on minimal Linux setups).
    """
    ...
    try:
        ...
        return
    if families and FONT_FAMILY not in families:
        for cand in _UI_FONT_FALLBACKS:
            if cand in families:
                FONT_FAMILY = cand
                break
...
def enable_dpi_awareness():
    ...
    Windows-only: on Linux the display server / desktop environment already
    handles scaling for Tk applications.
    """
    if not IS_WINDOWS:
        return
    try:
...
TRANSLATIONS = {
    "es": {
        ...
        "err_screenshot": ("No se pudo capturar la pantalla. En Wayland "
                           "instala una herramienta de captura (grim, "
                           "gnome-screenshot o spectacle)."),
        "hint_linux_root": ("(En Linux los atajos globales requieren "
                            "privilegios de administrador: ejecuta la app "
                            "con sudo)."),
    ...
    "en": {
        ...
        "err_screenshot": ("Could not capture the screen. On Wayland install "
                           "a capture tool (grim, gnome-screenshot or "
                           "spectacle)."),
        "hint_linux_root": ("(On Linux global hotkeys require administrator "
                            "privileges: run the app with sudo)."),
...
# Screen capture with Linux fallbacks
# ----------------------------------------------------------------------------
# Pillow's ImageGrab covers X11 (via XCB). On Wayland-native sessions it
# fails, so we fall back to the desktop's native screenshot CLI: grim
# (wlroots/Sway), gnome-screenshot (GNOME), spectacle (KDE), or the generic
# X11 tools scrot / maim / ImageMagick's import.
_SCREENSHOT_TOOL_ORDER_WAYLAND = ["grim", "gnome-screenshot", "spectacle",
                                  "scrot", "maim", "import"]
_SCREENSHOT_TOOL_ORDER_X11 = ["scrot", "maim", "gnome-screenshot",
                              "spectacle", "grim", "import"]


def _capture_screen():
    """Return a PIL image of the whole screen; raise RuntimeError if every
    backend fails."""
    try:
        img = ImageGrab.grab()
        if img is not None and getattr(img, "width", 0) > 0:
            return img
    except Exception:
        pass
    ...
    order = (_SCREENSHOT_TOOL_ORDER_WAYLAND if wayland
             else _SCREENSHOT_TOOL_ORDER_X11)

    for name in order:
        tool = shutil.which(name)
        if not tool:
            continue
        tmp_fd, tmp_path = tempfile.mkstemp(prefix="ts_shot_", suffix=".png")
        os.close(tmp_fd)
        try:
            os.remove(tmp_path)  # some tools refuse to overwrite files
            if name == "grim":
                # grim writes the PNG to stdout with '-'
                r = subprocess.run([tool, "-"], capture_output=True,
                                   timeout=10)
                if r.returncode == 0 and r.stdout:
                    return Image.open(io.BytesIO(r.stdout)).convert("RGB")
            elif name == "gnome-screenshot":
                cmd = [tool, "-f", tmp_path]
            elif name == "spectacle":
                cmd = [tool, "-b", "-n", "-o", tmp_path]
            elif name == "import":
                cmd = [tool, "-window", "root", tmp_path]
            else:  # scrot / maim take the output path directly
                cmd = [tool, tmp_path]
    ...
    raise RuntimeError(tr("err_screenshot"))


# Window-manager helpers. Some Tk attributes (-alpha, -topmost) are not
# supported by every Linux WM / display server, so every call is defensive.
def set_window_alpha(win, value):
    try:
...
def fade_in(win, target=0.97, step=0.08):
    ...
    try:
        ...
        return  # alpha unsupported: show the window as-is
    if current < target:
        set_window_alpha(win, min(current + step, target))
        if win.winfo_exists():
...
def fade_out(win, callback, step=0.08):
    ...
    try:
        ...
        callback()  # alpha unsupported: skip straight to the callback
        return
    if current > 0.05:
        set_window_alpha(win, max(current - step, 0))
        if win.winfo_exists():
...
def apply_icon(win):
    ...
    try:
        if IS_WINDOWS:
            ...
            # X11 WMs ignore .ico windows; iconphoto with the PNG works both
            # for the taskbar and the title bar.
            if os.path.exists(ICON_PNG):
                if _icon_photo is None:
                    _icon_photo = tk.PhotoImage(file=ICON_PNG)
...
class ServiceController:
    ...
    def teardown_hooks(self):
        ...
        if lt is not None and lt.is_alive() and lt.ident and IS_WINDOWS:
        ...
        # On Linux/macOS the low-level evdev read loop cannot be interrupted
        # safely from outside: hotkeys were already disarmed above, so we just
        # mark the listener as stopped below; add_hotkey will spin a fresh
        # listener pair on the next start.
    ...
    def _do_start(self):
        try:
            ...
            if IS_LINUX and "root" in str(e).lower():
                err_text += " " + tr("hint_linux_root")
            self.state = STATE_STOPPED
...
class ScrollFrame(tk.Frame):
    ...
    def _bind_wheel(self, _e):
        # Windows/macOS deliver <MouseWheel>; X11 delivers Button-4 (up) /
        # Button-5 (down) presses instead.
...
class SettingsWindow(tk.Toplevel):
    ...
    def save_settings(self):
        ...
        try:
            ...
            # On Linux without root the scan-code tables cannot be built;
            # the hotkey format itself is fine, defer real probing to Start.
            if IS_LINUX and "root" in str(e).lower():
                pass
            else:
                messagebox.showwarning("Test Solver AI",
                                       tr("msg_hotkey_invalid"), parent=self)
                return
        except Exception:
            messagebox.showwarning("Test Solver AI", tr("msg_hotkey_invalid"),
                                   parent=self)
...
class SolverOverlay(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        # 'bottom_right_corner' is a valid cursor name on X11 and Windows.
        self.grip = tk.Label(
            self.container,
            text='//',
            bg=COLORS["header"],
...
def main():
    ...
    root = tk.Tk()
    resolve_app_fonts(root)
    root.withdraw()
    root.overrideredirect(True)
    set_window_alpha(root, 0)
    if IS_WINDOWS:
        # Windows-only helper attribute: keeps the hidden root out of the
